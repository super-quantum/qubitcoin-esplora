...
export default ({ t, scripthash, scripthashQR, scripthashTxs, goScripthash, openTx, spends, tipHeight, loading, ...S }) => {
  if (!scripthash) return;

  const { chain_stats, mempool_stats } = scripthash
      ...
      , shown_txs = scripthashTxs ? scripthashTxs.length : 0
      ...
      , est_prev_total_seen_count  = goScripthash.last_txids.length ? goScripthash.est_chain_seen_count + avail_mempool_txs : 0
      , est_curr_chain_seen_count = goScripthash.last_txids.length ? goScripthash.est_chain_seen_count + shown_txs : shown_txs - avail_mempool_txs
      , last_seen_txid = (shown_txs > 0 && est_curr_chain_seen_count < chain_stats.tx_count) ? last(scripthashTxs).txid : null
      , next_paging_txids = last_seen_txid ? [ ...goScripthash.last_txids, last_seen_txid ].join(',') : null
      , prev_paging_txids = goScripthash.last_txids.length ? goScripthash.last_txids.slice(0, -1).join(',') : null
      , prev_paging_est_count = goScripthash.est_chain_seen_count ? Math.max(goScripthash.est_chain_seen_count-perPage, 0) : 0

  const display_scripthash = scripthash.display_scripthash
      // in elements mode, only show QR codes for confidential addresses
      , is_confidential = process.env.IS_ELEMENTS && !!goScripthash.confidential_scripthash
        ...
        <div className="container">
          <div className="row">
            <div className="col-sm-8">
              <h1 className="font-h2">{t`Script Hash`}</h1>
              <div className="block-hash font-p1">
                <span className="text-gray">{display_scripthash}</span>
                { process.browser && <div className="code-button">
                  <div className="code-button-btn" role="button" data-clipboardCopy={display_scripthash}></div>
                </div> }
              </div>
            </div>
            {show_qr && <div className="col-sm-4">
              <img className="float-sm-right address-qr-code" src={ scripthashQR } />
            </div>}
          </div>
        </div>
      </div>
      <div className="container">
        ...
        { is_confidential && [
          <div>
            ...
            <div>{ goScripthash.confidential_scripthash }</div>
          </div>
        , <div>
            <div>{ t`Unconfidential` }</div>
            <div>{ goScripthash.scripthash }</div>
          </div>
        ] }
        ...
        <div>
          <div className="transactions">
            ...
            { scripthashTxs ? scripthashTxs.map(tx => txBox(tx, { openTx, tipHeight, t, spends, scripthash, ...S }))
                       : loader() }
          </div>
          ...
          <div className="load-more-container">
            <div>
              { loading ? <div className="load-more g-btn font-btn-2 disabled"><span>{t`Loading...`}</span><div>{loader("small")}</div></div>
                        : pagingNav(scripthash, last_seen_txid, est_curr_chain_seen_count, prev_paging_txids, next_paging_txids, prev_paging_est_count, t) }
            </div>
          </div>
...
const pagingNav = (scripthash, last_seen_txid, est_curr_chain_seen_count, prev_paging_txids, next_paging_txids, prev_paging_est_count, t) =>
  process.browser

? last_seen_txid != null &&
    <div className="load-more g-btn primary-btn font-btn-2" role="button" data-loadmoreTxsLastTxid={last_seen_txid} data-loadmoreTxsScripthash={scripthash.scripthash}>
      {t`Load more`}
    </div>

: [
    prev_paging_txids != null &&
      <a className="load-more" href={`scripthash/${scripthash.scripthash}?txids=${prev_paging_txids}&c=${prev_paging_est_count}`}>
        <div><img alt="" src={`${staticRoot}img/icons/arrow_left_blu.png`} /></div>
        <span>{t`Newer`}</span>
      </a>
  , next_paging_txids != null &&
      <a className="load-more" href={`scripthash/${scripthash.scripthash}?txids=${next_paging_txids}&c=${est_curr_chain_seen_count}`}>
        <span>{t`Older`}</span>
