export const blockTxsPerPage = 25;
export const addrTxsPerPage = 25;
export const blocksPerPage = 10;
export const maxMempoolTxs = 50;

export const nativeAssetId =
  process.env.NATIVE_ASSET_ID ||
  '6f0279e9ed041c3d710a9f57d0c02928416460c4b722ae3457a11eec381c526d';
export const nativeAssetLabel = process.env.NATIVE_ASSET_LABEL || 'QTC';
export const nativeAssetName = process.env.NATIVE_ASSET_NAME || 'Qubitcoin';

// Elements only
export const assetTxsPerPage = 25;
export const pegTxsPerPage = 25;
